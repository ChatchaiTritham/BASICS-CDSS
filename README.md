...
- **All six model families are computed (seed 42).** Static AUROC: LR 0.875, RF 0.936, GB 0.905, XGBoost 0.920, LSTM 0.955, TCN 0.878; temporal AUROC: 0.815 / 0.902 / 0.861 / 0.898 / 0.962 / 0.847. The LSTM has the highest static AUROC *and* is the most temporally robust (ΔAUROC +0.007); XGBoost is mid-pack — these computed rankings differ from the earlier literals and are reported as-is.
- **A leaderboard reversal.** Among the tabular families, random forest led on static AUROC (0.936) yet its ECE more than doubled under perturbation (0.058→0.122), while logistic regression — weakest on tabular AUROC — gave the lowest risk–coverage area (AURC 0.110) and the highest decision-curve net benefit at the 0.30 threshold (0.229).
